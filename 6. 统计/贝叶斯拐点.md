>**引理一**
设$\mathbf{Y} = (Y_1, Y_2,...,Y_n)$是从泊松分布Poisson分布Poi($\lambda$)中抽样的随机样本，其中$\lambda$作为贝叶斯模型下的模型参数服从Gamma分布：Gamma$(\alpha, \beta)$,下面证明$\lambda$的后延分布是Gamma$(\alpha+rn, \Sigma_{j=1}^{n}Y_j+\beta)$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1NDI0MTgzMTJdfQ==
-->