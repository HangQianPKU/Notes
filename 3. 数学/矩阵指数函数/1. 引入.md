...
例 2.4.4 设 $\boldsymbol{X}=\left(X_1, \cdots, X_n\right)$ 是从总体 $N\left(\mu, \sigma^2\right)$ 中抽取的随机样本, $\boldsymbol{\theta}=(\mu, \sigma)$, 求 $(\mu, \sigma)$ 的联合无信息先验.
解 给定 $\boldsymbol{X}$ 时, $\boldsymbol{\theta}$ 的对数似然函数是
$$
l(\boldsymbol{\theta} \mid \boldsymbol{x})=-\frac{n}{2} \ln 2 \pi-n \ln \sigma-\frac{1}{2 \sigma^2} \sum_{i=1}^n\left(x_i-\mu\right)^2 .
$$
记 $\boldsymbol{I}(\boldsymbol{\theta})=\left(I_{i j}(\boldsymbol{\theta})\right)_{2 \times 2}$, 则有
$$
\begin{aligned}
& I_{11}(\boldsymbol{\theta})=E^{\boldsymbol{X} \mid \boldsymbol{\theta}}\left\{-\frac{\partial^2 l(\boldsymbol{\theta} \mid \boldsymbol{x})}{\partial \mu^2}\right\}=\frac{n}{\sigma^2}, \\
& I_{22}(\boldsymbol{\theta})=E^{\boldsymbol{X} \mid \boldsymbol{\theta}}\left\{-\frac{\partial^2 l(\boldsymbol{\theta} \mid \boldsymbol{x})}{\partial \sigma^2}\right\}=-\frac{n}{\sigma^2}+\frac{3}{\sigma^4} E\left\{\sum_{i=1}^n\left(X_i-\mu\right)^2\right\}=\frac{2 n}{\sigma^2}, \\
& I_{12}(\boldsymbol{\theta})=I_{21}(\boldsymbol{\theta})=E^{\boldsymbol{X} \mid \boldsymbol{\theta}}\left\{-\frac{\partial^2 l(\boldsymbol{\theta} \mid \boldsymbol{x})}{\partial \mu \partial \sigma}\right\}=E\left\{\frac{2}{\sigma^3} \sum_{i=1}^n\left(X_i-\mu\right)\right\}=0,
\end{aligned}
$$
故有
$$
...
\frac{n}{\sigma^2} & 0 \\
0 & \frac{2 n}{\sigma^2}
\end{array}\right), \quad[\operatorname{det} \boldsymbol{I}(\boldsymbol{\theta})]^{\frac{1}{2}}=\frac{\sqrt{2} n}{\sigma^2},
$$
所以, $(\mu, \sigma)$ 的 Jeffreys 先验 (由于它是广义先验, 可以丢弁常数因子) 为
$$
\pi(\mu, \sigma)=\frac{1}{\sigma^2},
$$
即 $(\mu, \sigma)$ 的联合无信息先验为 $1 / \sigma^2$. 它的几个特例为:
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM2ODc0NjgwNCwtMTg0NTgxMTgyMSw3Nj
UxMDM0NzVdfQ==
-->